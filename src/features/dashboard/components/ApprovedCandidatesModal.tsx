...
import { X, MessageCircle } from 'lucide-react'; // Importando o ícone de mensagem
...
// Transforma "(51) 9 9479-6527" em "5551994796527"
const formatPhoneNumberForWhatsApp = (phone: string | null): string | null => {
  ...
  // Se o número tiver 11 dígitos (DDD + 9 + número), adiciona o código do Brasil (55)
  if (digitsOnly.length === 11) {
    return `55${digitsOnly}`;
  }
  // Se já tiver 13 dígitos (já com código do país), retorna como está
  if (digitsOnly.length === 13) {
    return digitsOnly;
  }
  
  // Retorna null se o número não estiver em um formato reconhecido
  return null; 
};
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-xs text-gray-500 uppercase border-b bg-gray-50">
                  ...
                  <th className="px-4 py-3 font-semibold text-center">Ações</th> {/* <-- NOVA COLUNA */}
                </tr>
              </thead>
              <tbody>
                {candidates.map((candidate) => {
                  ...
                  return (
                    <tr key={candidate.id} className="border-b hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-4 text-gray-600">{candidate.TRIAGEM}</td>
                      ...
                      <td className="px-4 py-4 text-center">
                        <a
                          ...
                          `}
                          // Previne o clique se não houver número
