...
import { LayoutDashboard, PlusCircle, Settings, LogOut, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const menuItems = [
    ...
    // AQUI ESTÁ A MUDANÇA: Ativamos o item de Configurações
    ...
    <div className={`hidden md:flex flex-col bg-white shadow-md transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      <nav className="flex-grow p-4 space-y-2">
        {menuItems.map((item) => (
          <button
            key={item.key}
            onClick={() => onNavigate(item.key)} // Agora todos os itens são navegáveis
            className={`flex items-center w-full p-3 rounded-lg font-medium transition-colors ${
              item.active ? 'text-gray-600 bg-gray-100' : 'text-gray-500 hover:bg-gray-100'
